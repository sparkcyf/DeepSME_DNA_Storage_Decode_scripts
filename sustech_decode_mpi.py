...
if __name__ == '__main__':
    ...
    # Distribute payloads among processes
    payloads_per_process = len(payloads) // size
    remainder = len(payloads) % size

    if rank < remainder:
        start = rank * (payloads_per_process + 1)
        end = start + payloads_per_process + 1
    else:
        start = rank * payloads_per_process + remainder
        end = start + payloads_per_process

    local_payloads = payloads[start:end]
    local_refs = refs[start:end]
    local_source_refs = source_file_ref[start:end]

    # Preallocate local decoded results
    local_txt_decode_res = []
    local_pic_decode_res = []
    local_fail_info = []

    # Decode local payloads
    for i, payload in enumerate(local_payloads):
        global_index = start + i  # To keep track of the original index
        total_segments += 1  # Count total number of segments processed
        data = hedges.combine_to_letters(payload)
        ref = local_refs[i][57:-11]
        ref = ref[:40]+ref[45:85]+ref[90:130]+ref[135:]

        if len(payload) != len(ref):
            decode_fail_num += 1
            total_failed_segments += 1
            fail_reason = f'Length mismatch: ref={len(ref)}, payload={len(payload)}'
            local_fail_info.append({'index': global_index, 'success': False, 'reason': fail_reason})
            print(f'Process {rank}: Sequence {global_index} failed - {fail_reason}')
            continue

        # res = hedges.decode(data)  # Assuming decode returns list of hypotheses
        ...
        if decode_success:
            ...
            if mismatched_bytes > 0:
                ...
                decode_fail_num += 1
                total_failed_segments += 1
                fail_reason = f'{mismatched_bytes}/{len(ref_bytes)} bytes mismatched ({mismatch_percentage:.2f}%)'
                local_fail_info.append({'index': global_index, 'success': False, 'reason': fail_reason})
                print(f'Process {rank}: Sequence {global_index} failed - {fail_reason}')
            else:
                print(f'Process {rank}: Sequence {global_index} decoded successfully.')
        else:
            total_mismatched_bytes += len(ref_bytes)
            decode_fail_num += 1
            total_failed_segments += 1
            fail_reason = f'Decode failed completely! {len(ref_bytes)}/{len(ref_bytes)} bytes mismatched (100.00%)'
            local_fail_info.append({'index': global_index, 'success': False, 'reason': fail_reason})
            print(f'Process {rank}: Sequence {global_index} failed - {fail_reason}')

        # Append decoded results
        if global_index <= 54:
            local_txt_decode_res.append(rs_decode_res)
        else:
            local_pic_decode_res.append(rs_decode_res)

    # Gather results at root
    all_txt_decode_res = comm.gather(local_txt_decode_res, root=0)
    all_pic_decode_res = comm.gather(local_pic_decode_res, root=0)
    all_fail_info = comm.gather(local_fail_info, root=0)
    all_total_bytes = comm.reduce(total_bytes, op=MPI.SUM, root=0)
    all_total_mismatched_bytes = comm.reduce(total_mismatched_bytes, op=MPI.SUM, root=0)
    all_total_segments = comm.reduce(total_segments, op=MPI.SUM, root=0)
    all_total_failed_segments = comm.reduce(total_failed_segments, op=MPI.SUM, root=0)
    all_decode_fail_num = comm.reduce(decode_fail_num, op=MPI.SUM, root=0)

    if rank == 0:
        # Flatten the gathered lists
        flat_txt_decode_res = list(itertools.chain.from_iterable(all_txt_decode_res))
        flat_pic_decode_res = list(itertools.chain.from_iterable(all_pic_decode_res))
        flat_fail_info = list(itertools.chain.from_iterable(all_fail_info))

        # Write decoded results to files
        with open(sustech_pic_output, 'wb') as f:
            for item in flat_pic_decode_res:
                f.write(item)
        with open(sustech_txt_output, 'wb') as f:
            for item in flat_txt_decode_res:
                f.write(item)
        ...
        total_bytes_all = len(original_pic) + len(original_txt)
        weighted_accuracy = (pic_accuracy * len(original_pic) + txt_accuracy * len(original_txt)) / total_bytes_all

        print('\nDecoding Summary:')
        print(f'Total segments processed: {all_total_segments}')
        print(f'Failed segments: {all_total_failed_segments} ({(all_total_failed_segments/all_total_segments)*100:.2f}%)')
        print(f'Total bytes processed during decoding: {all_total_bytes}')
        print(f'Total mismatched bytes during decoding: {all_total_mismatched_bytes}/{all_total_bytes} ({(all_total_mismatched_bytes/all_total_bytes)*100:.2f}%)')
        ...
        print(f'Failed indices: {[item["index"] for item in flat_fail_info]}')
        
        # # Save failed indices and reasons to CSV
        # csv_filename = 'decode_failures.csv'
        # with open(csv_filename, 'w', newline='') as csvfile:
        #     fieldnames = ['index', 'success', 'reason']
        #     writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        #     writer.writeheader()
        #     for fail in flat_fail_info:
        #         writer.writerow(fail)
        # print(f'Failed indices and reasons have been saved to {csv_filename}')
